...
# Fit KDE to all collected left prominences
left_prominences_array = np.array(all_left_prominences_after_2s).reshape(-1, 1)
kde = KernelDensity(kernel='gaussian', bandwidth=0.01).fit(left_prominences_array)
...
# Determine threshold as the 80th percentile of left prominence
threshold = np.percentile(all_left_prominences_after_2s, 80)
...
plt.ylabel('Density')
plt.title('Left Prominence Distribution Across All Datasets (KDE Fit)')
plt.legend()
plt.savefig("global_left_prominence_distribution_kde.png")
plt.show()

print(f"Global Left Prominence Threshold (KDE 80th Percentile): {threshold:.4f}")
