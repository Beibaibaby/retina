...
#datasets = ['plot_all_rs_norm_null1_traces_final', 'plot_all_rs_norm_null2_traces_final','plot_all_rs_norm_null4_traces_final','plot_all_rs_norm_pref1_traces_final','plot_all_rs_norm_pref2_traces_final','plot_all_rs_norm_pref4_traces_final']
...
# Collect all left prominences after 2 seconds from all datasets
all_left_prominences_after_2s = []
...
for dataset_name in datasets:
    ...
    for i in range(trace_data.shape[1]):
        ...
        all_left_prominences_after_2s.extend(left_prominences_after_2s)

# Fit GMM to all collected left prominences
left_prominences_array = np.array(all_left_prominences_after_2s).reshape(-1, 1)
gmm = GaussianMixture(n_components=2, random_state=42).fit(left_prominences_array)
...
plt.hist(all_left_prominences_after_2s, bins=100, color='skyblue', edgecolor='black', alpha=0.7, density=True)
x = np.linspace(min(all_left_prominences_after_2s), max(all_left_prominences_after_2s), 1000).reshape(-1, 1)
...
plt.ylabel('Density')
plt.title('Left Prominence Distribution Across All Datasets (GMM Fit)')
plt.legend()
plt.savefig("global_left_prominence_distribution_gmm.png")
plt.show()
...
for dataset_name in datasets:
    ...
    plt.tight_layout()
    plt.savefig(f"{dataset_name}_new.png")
    plt.close()
